...
Param(
  ...
  [ValidateSet("windows","linux","osx","android","openharmony","browser","wasi")][string]$os,
  [switch]$coverage,
  [string]$testscope,
  [switch]$testnobuild,
...
function Get-Help() {
  ...
  Write-Host "                                 [Default: Debug]"
  Write-Host "  -os                            Target operating system: windows, linux, osx, android, openharmony, wasi or browser."
