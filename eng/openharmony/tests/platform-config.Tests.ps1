...
Describe 'OpenHarmony runtime platform configuration' {
    ...
    It 'does not probe Linux NUMA syscalls inside the OpenHarmony app sandbox' {
        $numaSupport = Get-Content -LiteralPath (Join-Path $repoRoot 'src\coreclr\gc\unix\numasupport.cpp') -Raw
        $openHarmonySafeGuard = '#if defined\(TARGET_LINUX\) && !defined\(TARGET_ANDROID\) && !defined\(TARGET_OPENHARMONY\)'

        ([regex]::Matches($numaSupport, $openHarmonySafeGuard)).Count | Should -Be 5
        $numaSupport | Should -Not -Match '(?m)^#ifdef TARGET_LINUX\r?$'
    }
